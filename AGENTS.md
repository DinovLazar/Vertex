...
- **AI:** `@anthropic-ai/sdk` — chat widget (`/api/chat`) + content generator (`src/lib/contentGenerator/`). Email: **Resend**. Animation: `motion` (import from `motion/react`), gsap, three/@react-three/fiber (Silk hero). The marketing Plasma hero is a pre-rendered looping video (`public/heroes/`, regenerated by `scripts/render-plasma-hero.mjs`) — `ogl` was removed 2026-08-23.
