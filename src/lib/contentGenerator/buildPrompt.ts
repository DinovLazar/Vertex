...
Reference post 3, an opening that establishes the problem fast:
...
export function buildSystemPrompt(args: BuildPromptArgs): string {
  ...
  const serviceLink = targetService
    ? `- Include 1 internal link to ${targetService} inside the body (using a markDef with _type: "link" and href: "${targetService}"). Pick a natural place, and don't force it. The link text should describe what that page is, not be generic "click here".`
...
Key rules, matching the tone of these reference passages from existing Vertex posts:
${REFERENCE_PASSAGES}
...
- Never use em dashes (—) anywhere in the post, in either language. Use a comma, a colon, or a separate sentence instead. The same goes for the title, the excerpt, the meta description and both social captions.
