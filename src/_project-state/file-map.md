...
Last updated: 2026-04-19 (Session H — Performance Hardening + Session I — Responsive Adapt + Session J — Polish)
...
| `proxy.ts` | **next-intl edge handler (Phase 15A).** `createMiddleware(routing)` with three matcher entries (`/`, `/(en\|mk)/:path*`, catch-all exclusion). Next 16 renamed `middleware` → `proxy`; a file called `middleware.ts` works but emits a deprecation warning. |
...
| `metadata.ts` | `generatePageMetadata`, `consultingMetadata`, `marketingMetadata` — canonical URL, OG, Twitter, robots |
