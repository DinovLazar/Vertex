...
## 2026-08-23 — Session: Plasma Video Hero (dependency removal)
- **Removed: `ogl`** (was `^1.0.11`). Its only consumer, `src/components/backgrounds/Plasma.tsx`, was deleted — the marketing hero's plasma is now a pre-rendered looping video pair under `public/heroes/` (generated by `scripts/render-plasma-hero.mjs`), played by the rewritten `BackgroundPlasma.tsx`. `src/types/ogl.d.ts` (hand-rolled ambient types) deleted with it.
- The generator's deps are **on-demand, not committed** (same policy as `capture-projects.mjs`): `npm install --no-save playwright ffmpeg-static` (used `playwright@1.62.1`, `ffmpeg-static@5.3.0` this session) + `npx playwright install chromium`. Note `--no-save` installs are pruned by any subsequent npm install/uninstall — install both in one command.
- CSS token removed: `--plasma-opacity` (globals.css) — the light-mode 0.4 damper is baked into the light video's pixels.
