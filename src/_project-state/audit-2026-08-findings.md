...
| ID | File | Status |
|---|---|---|
| C-3 | `src/app/not-found.tsx` | **Superseded 2026-08-18 (Phase 18).** The `lang`-less wrapper is still real, but the finding's scope was wrong: this file is not the site's only 404 boundary. Its remit is now just the locale-less paths the proxy never rewrites (`/admin/*`, asset-like URLs); every public 404 is served by the new `src/app/[locale]/(site)/not-found.tsx`, which renders inside `[locale]/layout.tsx` and therefore *does* carry `<html lang>`. The root-layout document-shell fix is still the correct fix for the remaining non-public routes, and is still not attempted. |
| C-4 | `src/app/[locale]/not-found.tsx` | **Wrong — retracted 2026-08-18 (Phase 18).** The "confirmed dead code" verdict came from reading *curled* HTML, which shows a bare `<html id="__next_error__">` shell for any 404 in dev **and** prod, because Next streams the 404 body in through the RSC payload rather than the initial markup. The locale-tree boundary was executing the whole time. Re-checked in a browser against `next build`: a locale 404 renders with `lang` set, localized copy, Navbar and Footer. Lesson: never conclude which 404 boundary won from `curl` output alone — inspect the rendered DOM. |
...
### Part 5 finding — `Disallow` + `noindex` on the same URLs is self-defeating

`/privacy` and `/thank-you` currently carry **both** `noIndex: true` in their
page metadata AND a `Disallow` in robots.txt. That combination does not do what
it looks like it does: a crawler that obeys the `Disallow` never fetches the
page, so it never sees the `noindex` directive. Google can then still index the
bare URL if anything links to it, showing a title with no snippet — the exact
outcome the `noindex` was meant to prevent.

The textbook-correct configuration is to pick one:
- **Preferred** — keep `noIndex: true`, drop the two `/privacy` and two
  `/thank-you` lines from `DISALLOW` in `src/app/robots.ts`. Google crawls,
  reads `noindex`, and drops the pages cleanly.
- Or keep the `Disallow` and remove `noIndex` from the page metadata.

**Left as-is deliberately.** The Phase 17 Definition of Done explicitly requires
robots.txt to disallow `/privacy` and `/thank-you`, so changing it unilaterally
would fail a stated acceptance criterion. Impact is low — these are pages nobody
searches for. Flagged for Goran to decide; the fix is deleting four lines.

Related, also left as-is: `/studio` and `/admin` stay in `DISALLOW` even though
the DoD says robots should disallow "only" `/api/`, `/privacy` and `/thank-you`.
Removing them would re-expose the Sanity Studio and the admin panel to crawlers,
which the July SEO session deliberately closed. "Only" was read as scoping the
noindex-page list, not as an instruction to un-block private surfaces.
